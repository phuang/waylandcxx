...
const struct wl_keyboard_listener Keyboard::listener_ = {
  ...
  Keyboard::OnEnterThunk,
  Keyboard::OnLeaveThunk,
  Keyboard::OnKeyThunk,
  Keyboard::OnModifiersThunk,
  Keyboard::OnRepeatInfoThunk,
};
...
Keyboard::~Keyboard() {
  wl_keyboard_release(id());
}

void Keyboard::OnKeymap(struct wl_keyboard* keyboard,
                        uint32_t format,
                        int32_t fd,
                        uint32_t size) {
...
void Keyboard::OnEnter(struct wl_keyboard* keyboard,
                       uint32_t serial,
...
void Keyboard::OnLeave(struct wl_keyboard* keyboard,
                       uint32_t serial,
                       struct wl_surface* surface) {
  fprintf(stderr, "%s this=%p\n", __PRETTY_FUNCTION__, this);
}

void Keyboard::OnKey(struct wl_keyboard* keyboard,
                     uint32_t serial,
                     uint32_t time,
                     uint32_t key,
                     uint32_t state) {
...
void Keyboard::OnModifiers(struct wl_keyboard* keyboard,
                           uint32_t serial,
...
void Keyboard::OnRepeatInfo(struct wl_keyboard *wl_keyboard,
                            int32_t rate,
                            int32_t delay) {
  fprintf(stderr, "%s this=%p\n", __PRETTY_FUNCTION__, this);
}
